    ...
    for section in sections:
        ...
        for task in tasks:
            ...
            if not assignments.exists():
                raise ValidationError({
                    'assignments': f"'{task.title}' vazifasida kamida bitta tayinlash bo'lishi kerak."
                })
            ...
            if missing_drivers:
                raise ValidationError({
                    'assignments': f"'{task.title}' vazifasida avtomobil transportiga haydovchi tayinlanmagan."
                })
...
def approve_main_duty(main_duty, approved_by):
    ...
    if main_duty.status != MainDutyStatus.SENT_FOR_APPROVAL:
        raise ValidationError({'status': "Faqat tasdiqlashga yuborilgan navbatchilikni tasdiqlash mumkin."})
...
def reject_main_duty(main_duty, rejected_by, reason):
    ...
    if main_duty.status != MainDutyStatus.SENT_FOR_APPROVAL:
        raise ValidationError({'status': "Faqat tasdiqlashga yuborilgan navbatchilikni rad etish mumkin."})
